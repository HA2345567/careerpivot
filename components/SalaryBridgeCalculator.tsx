...
export const SalaryBridgeCalculator = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const data = await calculatorService.getLatestScenario();
        if (data) {
      ...
      } catch (e) { console.error("Failed to load calculator settings", e); }
    };
    load();
  }, []);

  // Debounced auto-save (Optional, but let's stick to saving on Generate to reduce DB writes for now, or just save inputs)
  // For 'Deep Core', manual save or save-on-generate is often better than spamming the DB on slider drag.
  // I will save when "Generate Plan" is clicked, but we can also add a small effect for local state persistence if needed.
  // For now, let's keep the user inputs in memory and only persist the "Scenario" when they actually generate or explicit save.
  // However, removing the auto-save effect that was writing to localStorage is correct.
  ...
  const handleGeneratePlan = async () => {
    ...
    // setFinancialPlan(null); // Keep old one visible while regenerating

    try {
      let plan: FinancialPlan | null = null;
      if (process.env.API_KEY) {
        const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
        const prompt = `Act as a ruthless financial CFO for a startup (the user).
        
        DATA:
        Current Salary: $${currentSalary}
        Burn Rate: $${monthlyExpenses}/mo
        Runway Goal: ${transitionMonths} months (Total: $${bridgeAmount})
        Target Salary: $${targetSalary}
        Gap Analysis: ${payCutImpact >= 0 ? 'Surplus' : 'Deficit'} of $${Math.abs(payCutImpact)}/mo
        
        OUTPUT REQUIREMENTS:
        1. "Savings Strategy": Don't just say "save more". Give a specific mathematical target (e.g. "Auto-transfer $X every Friday").
        2. "Expense Audits": Identify 3 specific "Bleed" areas typical for this income bracket (e.g. "Premium Subscriptions", "Unused Gym", "Friday DoorDash"). Be direct.
        3. "Safety Net Assessment": Give a risk score (High/Med/Low) and ONE sentence on why.
        4. "Bridge Tactics": Suggest 1 specific, unconventional way to generate cash during the gap (e.g. "Sell 3 months of consulting upfront").
        `;
      ...
      if (plan) {
        ...
        // Persist to DB
        const { calculatorService } = await import('../services/calculatorService');
        await calculatorService.saveScenario({
          currentSalary,
          monthlyExpenses,
          transitionMonths,
          targetSalary
