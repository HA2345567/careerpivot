...
interface DailyAccountabilityProps {
  mode?: 'vent' | 'action';
}

export const DailyAccountability = ({ mode = 'vent' }: DailyAccountabilityProps) => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      sender: 'bot',
      text: mode === 'vent'
        ? "Hey! I'm your pivot coach. 🤖 Feeling stuck or frustrated today? Vent here. I'll turn your rant into one tiny, actionable step."
        : "Let's get moving! 🚀 I'm analyzing your roadmap to find the smallest possible step you can take right now.",
      timestamp: new Date()
    }
  ]);
  ...
  // Auto-trigger action mode - wrapped in try/catch just in case
  useEffect(() => {
    try {
      if (mode === 'action' && !initialized.current) {
        initialized.current = true;
        handleSend("Generate a micro-step for me based on my roadmap", true);
      }
    } catch (e) {
  ...
  const handleSend = async (textOverride?: string, isHiddenCommand: boolean = false) => {
