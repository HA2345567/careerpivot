...
export const Card: React.FC<CardProps> = ({ children, className = '', hoverEffect = false }) => {
  return (
    <div className={`rounded-xl border border-white/5 bg-zinc-900/40 backdrop-blur-md shadow-xl ${hoverEffect ? 'hover:border-white/10 hover:bg-zinc-900/60 transition-all duration-500' : ''} ${className}`}>
      {children}
    </div>
  );
};
